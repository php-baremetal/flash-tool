...
- Hold no hard-coded knowledge of extensions or hardware. The extensions, their build flags and
  ...
  PHP 8.3; the tool gains whatever php-esp32 adds without a change here.
...
- **[BurntSushi/toml](https://github.com/BurntSushi/toml)** for decoding the config and the php-esp32
  descriptors.
...
interactive prompts (a small terminal prompter behind an interface, so command logic is tested without
a TTY).
...
per-type rule there is picked up by phpflash with no code change, and `php-esp32/scripts/check-manifest.py`
keeps the manifest in step with the actual CMake build.
...
in-source `sdkconfig` and builds the wrong architecture. An `embedded` project additionally passes
`-DPHP_EMBED_SRC`, so php-esp32 packs the source into a read-only image; a networked project can pass
`-DPHP_NET_DNS`, and a full-openssl `tls` project `-DPHP_TLS_CAFILE` and `-DPHP_OPENSSL_CONF`.

Two guardrails sit around `flash`. Before writing, phpflash probes the connected chip and refuses if
its target does not match the project board's family, so an image built for one chip is not pushed onto
another (overridable with `--force`, and skipped silently if the probe cannot run, since esptool
verifies the chip during the write anyway). After a `microsd` flash, it erases the `storage` partition,
because a `microsd` project ships no embedded image and the firmware prefers an embedded image over the
card, so a leftover one from an earlier build would otherwise shadow it.
...
- **Ports and USB**: glob `/dev/ttyACM*` and `/dev/ttyUSB*` (never opening a device) and read the USB
  descriptors from sysfs.
- **Chip**: run esptool's `flash_id` to read the chip type and target, revision, flash size, MAC and
...
- **Boards**: map the chip's target to a family and list its boards, each annotated with the network
...
- **manifest**: parses php-esp32's descriptors and resolves a project's effective set into the
  `-D<flag>` list, rejecting unmet requirements; exposes each board's peripherals and its family
  target.
- **build**: checks the exact `idf.py` argument list, that fetches run first, the embed, DNS and TLS
  argument derivation, the storage-partition erase, and the CA-bundle provisioning; the progress
  renderer is checked on synthetic ninja output.
- **discover**: parses esptool output and the discovery-firmware report, derives the chip target
  (dropping the package suffix) and built-in radio, and matches a chip to a family.
